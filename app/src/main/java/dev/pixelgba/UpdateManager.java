    ...
    void check(boolean announce){
        new Thread(()->{
            try {
                ...
                int code=u.optInt("versionCode",0);String name=u.optString("versionName","");String url=u.optString("apkUrl","");
                ...
                activity.runOnUiThread(()->offer(code,name,url,u.optString("changelog","")));
            }catch(Exception e){if(announce)activity.runOnUiThread(()->activity.toast("Update check failed: "+e.getMessage()));}
        },"ota-check").start();
    }
    void offer(int code,String name,String url,String notes){
        ...
        new AlertDialog.Builder(activity).setTitle("UPDATE AVAILABLE").setMessage(message).setNegativeButton("LATER",null).setPositiveButton("DOWNLOAD",(d,w)->download(code,name,url)).show();
    }
    void download(int code,String name,String url){
        try{
